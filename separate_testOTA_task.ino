...
void sendMqtt(const char* Data){
  if(client.connected()){
    if(millis() - Tinit > Tset){
       client.publish(TEST,Data);
       Serial.println(String("Send: ")+Data);
       Tinit = millis();
    }
  }else{
    mqttconnect();
...
void loop() {
    ...
    sendMqtt("Send and wait for OTA");
}
